...
std::shared_ptr<RobotWebRTCClient> RobotWebRTCClientFactory::Create() {
    auto signaling_server = std::make_shared<SignalingServer>(signaling_config_->GetSignalingUrl());
    signaling_server->Start();

    auto client_state_manager = std::make_shared<ClientStateManager>(signaling_server);

    auto webrtc_client = std::make_shared<RobotWebRTCClient>(robot_id_, user_id_, client_state_manager);
