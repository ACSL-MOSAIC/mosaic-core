...
WebSocketClientImpl::WebSocketClientImpl() : m_connected(false) {}
...
void WebSocketClientImpl::setOnConnected(OnConnectedCallback callback) {
    m_onConnected = std::move(callback);
}

void WebSocketClientImpl::setOnDisconnected(OnDisconnectedCallback callback) {
    m_onDisconnected = std::move(callback);
}

void WebSocketClientImpl::setOnMessage(OnMessageCallback callback) {
    m_onMessage = std::move(callback);
}

void WebSocketClientImpl::setOnError(OnErrorCallback callback) {
...
void WebSocketClientImpl::connectInternal() {
    // WebSocket 클라이언트 설정
...
void WebSocketClientImpl::send(const std::string& message) const {
    if (!m_connected) {
...
void WebSocketClientImpl::setupEventHandlers() {
    // 메시지 수신 핸들러
...
void WebSocketClientImpl::handleMessage(const websocket_incoming_message& message) const {
    try {
...
void WebSocketClientImpl::handleClose(websocket_close_status close_status,
                                      const std::string& reason,
