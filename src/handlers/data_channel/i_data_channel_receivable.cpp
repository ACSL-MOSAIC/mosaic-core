...
Json::Value IDataChannelReceivable::ConvertDataBufferToJson(const webrtc::DataBuffer &buffer) {
    ...
    std::string errors;
    std::istringstream message_stream(std::string(buffer.data.data<char>(), buffer.data.size()));
    if (!Json::parseFromStream(reader_builder, message_stream, &json_data, &errors)) {
