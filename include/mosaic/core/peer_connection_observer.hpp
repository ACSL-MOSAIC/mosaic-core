//
// Created by yhkim on 25. 6. 5.
//

#ifndef MOSAIC_CORE_PEER_CONNECTION_OBSERVER_HPP
#define MOSAIC_CORE_PEER_CONNECTION_OBSERVER_HPP

#include <api/peer_connection_interface.h>

#include "mosaic_connector.hpp"

namespace mosaic::core {
class PeerConnectionObserver : public webrtc::PeerConnectionObserver {
  public:
    explicit PeerConnectionObserver(const std::shared_ptr<MosaicConnector>& mosaic_connector,
                                    const std::shared_ptr<PeerConnectionManager>& pc_manager)
        : mosaic_connector_(mosaic_connector), pc_manager_(pc_manager) {}

    ~PeerConnectionObserver() override = default;

    // Triggered when a remote peer opens a data channel.
    void OnDataChannel(webrtc::scoped_refptr<webrtc::DataChannelInterface> data_channel) override;

    // A new ICE candidate has been gathered.
    void OnIceCandidate(const webrtc::IceCandidateInterface* candidate) override;

    // Called any time the standards-compliant IceConnectionState changes.
    void OnStandardizedIceConnectionChange(webrtc::PeerConnectionInterface::IceConnectionState new_state) override;

    // Called any time the IceGatheringState changes.
    void OnIceGatheringChange(webrtc::PeerConnectionInterface::IceGatheringState new_state) override;

    // Triggered when the SignalingState changed.
    void OnSignalingChange(webrtc::PeerConnectionInterface::SignalingState new_state) override;

    // Called any time the PeerConnectionState changes.
    void OnConnectionChange(webrtc::PeerConnectionInterface::PeerConnectionState new_state) override;

  private:
    std::shared_ptr<MosaicConnector> mosaic_connector_;
    std::shared_ptr<PeerConnectionManager> pc_manager_;
};
}  // namespace mosaic::core

#endif  // MOSAIC_CORE_PEER_CONNECTION_OBSERVER_HPP
