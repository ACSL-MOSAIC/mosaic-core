...
class PyIConfigurableConnectorFactory : public IConfigurableConnectorFactory {
  public:
    ...
    std::shared_ptr<IConfigurableConnector> CreateConnector() override {
        py::gil_scoped_acquire gil;
        return py::cast(this).attr("create_connector")().cast<std::shared_ptr<IConfigurableConnector>>();
    }
};
...
class PyADCHandlerConfigurer : public ADCHandlerConfigurer {
  public:
    ...
    void SetHandler(const std::shared_ptr<IDataChannelHandler>& h) { handler_ = h; }
};
...
class PyAMTHandlerConfigurer : public AMTHandlerConfigurer {
  public:
    ...
    void SetHandler(const std::shared_ptr<IMediaTrackHandler>& h) { handler_ = h; }
};
...
class PyAParallelDCHandlerConfigurer : public AParallelDCHandlerConfigurer {
  public:
    ...
    void SetHandlers(const std::vector<std::shared_ptr<IDataChannelHandler>>& hs) { handlers_ = hs; }
};
...
class PyAutoConfigurer : public AutoConfigurer {
  public:
    ...
    std::shared_ptr<MosaicConfig> GetMosaicConfig() const { return mosaic_config_; }

    void AddConnector(const std::shared_ptr<IConfigurableConnector>& c) {
        configurable_connectors_.push_back(c);
    }

    std::vector<std::shared_ptr<IConfigurableConnector>> GetConnectors() const { return configurable_connectors_; }
...
void bind_auto_configurer(py::module_& m) {
    ...
    py::class_<IConfigurableConnector, std::shared_ptr<IConfigurableConnector>, PyIConfigurableConnector>(
        ...
        .def("set_config", &IConfigurableConnector::SetConfig)
        .def("get_config", &IConfigurableConnector::GetConfig)
    ...
    py::class_<IConfigurableConnectorFactory, std::shared_ptr<IConfigurableConnectorFactory>, PyIConfigurableConnectorFactory>(
        m, "IConfigurableConnectorFactory")
        .def(py::init<>())
        .def("get_connector_type", [](IConfigurableConnectorFactory&) -> std::string { return {}; })
        .def("create_connector", [](IConfigurableConnectorFactory&) -> std::shared_ptr<IConfigurableConnector> { return nullptr; });

    // ADCHandlerConfigurer (abstract, 상속 IConfigurableConnector)
    // handler 프로퍼티: Python 서브클래스의 configure()에서 self.handler = <handler> 로 설정
    py::class_<ADCHandlerConfigurer, std::shared_ptr<ADCHandlerConfigurer>, IConfigurableConnector, PyADCHandlerConfigurer>(
        m, "ADCHandlerConfigurer")
        .def(py::init<>())
        ...
        .def_property(
            "handler",
            &ADCHandlerConfigurer::GetHandler,
    ...
    py::class_<AMTHandlerConfigurer, std::shared_ptr<AMTHandlerConfigurer>, IConfigurableConnector, PyAMTHandlerConfigurer>(
        m, "AMTHandlerConfigurer")
        .def(py::init<>())
    ...
    py::class_<AutoConfigurer, PyAutoConfigurer>(m, "AutoConfigurer")
        .def(py::init<>())
        .def("auto_configure", &AutoConfigurer::AutoConfigure)
        ...
        .def_property_readonly("mosaic_config", [](AutoConfigurer& self) {
            return static_cast<PyAutoConfigurer&>(self).GetMosaicConfig();
        })
        .def_property_readonly("configurable_connectors", [](AutoConfigurer& self) {
            return static_cast<PyAutoConfigurer&>(self).GetConnectors();
        })
        ...
        .def_property_readonly("dc_handler_map", [](AutoConfigurer& self) {
            return static_cast<PyAutoConfigurer&>(self).GetDcHandlerMap();
        })
    ...
    m.def("register_configurable_connector",
